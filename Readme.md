...
1. Install CMake
2. Launch CMake
	1. Specify where is the source code: 
		"Where is the source code": ....../ifc2x3-SDK
	2. Specify where to build the binaries:
		"where to build the binaries": ....../ifc2x3-SDK
...
3. inside the ifc2x3-SDK folder create a sub-folder called build (it can be any name and can even outside this ifc2x3-SDK folder).
4. from this directory call CMake to generate the makefiles using the following command:
   ...
   _Note_ : Solaris needs an additional flag to compile (at least with SunStudio 11) you will need to issue the following command :
...
7. on successfull build you should have the libraries and the binaries located in your ifc2x3-SDK/build folder
...
Follow 5.1 up to step 4 but choosing 'MinGW - Makefiles' obviously.
Then follow 4.1 from step 6 using 'mingw32-make.exe' instead of 'make'.
...
There is a special target that CMake will create called ```Documentation``` that will generate the documention provided you have installed the tool called doxygen. It can be downloaded from [http://doxygen.org](http://doxygen.org). 
Documentation will be generated into the ifc2x3-SDK/build/doc/html directory and the entry point is the 'index.html' file.
