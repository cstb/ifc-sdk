...
const std::string &IfcStructuralAction::type() {
...
Step::Bool IfcStructuralAction::getDestabilizingLoad() {
...
void IfcStructuralAction::setDestabilizingLoad(Step::Bool value) {
...
void IfcStructuralAction::setCausedBy(const Step::RefPtr< IfcStructuralReaction > &value) {
    m_causedBy = value;
    m_causedBy->m_causes.insert(this);
}
...
bool IfcStructuralAction::init() {
    ...
    if (arg == "$" || arg == "*") {
        ...
        m_causedBy = static_cast< IfcStructuralReaction * > (m_expressDataSet->get(atoi(arg.c_str() + 1)));
    }
    return true;
}
...
void IfcStructuralAction::copy(const IfcStructuralAction &obj, const CopyOp &copyop) {
    ...
    setCausedBy(copyop(obj.m_causedBy.get()));
    return;
}
