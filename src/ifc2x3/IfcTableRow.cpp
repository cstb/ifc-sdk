...
IfcTableRow::IfcTableRow(Step::SPFData *args) : Step::BaseObject(args) {
  m_rowCells.setUnset(true);
  m_isHeading = getUnset(m_isHeading);
  m_ofTable = NULL;
}
...
Step::StepList< Step::RefPtr< IfcValue > > &IfcTableRow::getRowCells() {
...
void IfcTableRow::setRowCells(const Step::StepList< Step::RefPtr< IfcValue > > &value) {
  m_rowCells = value;
}

Bool IfcTableRow::getIsHeading() {
  if (Step::BaseObject::inited()) {
    return m_isHeading;
  }
  else {
    return getUnset(m_isHeading);
  }
}

void IfcTableRow::setIsHeading(Bool value) {
  m_isHeading = value;
}
...
void IfcTableRow::setOfTable(const Step::RefPtr< IfcTable > &value) {
  m_ofTable = value;
}
...
bool IfcTableRow::init() {
  ...
  if (inverses) {
    m_ofTable = static_cast< IfcTable * > (m_model->getObjectById((*inverses)[0]));
  }
  return true;
}

IFC2X3_DLL_DEF Step::ClassType IfcTableRow::s_type = new Step::ClassType_class("IfcTableRow");
IfcTableRow_Factory::IfcTableRow_Factory() {
}
...
void IfcTableRow_Factory::clear(bool b) {
}

std::map<Step::StepId,Step::BaseObject*>::iterator IfcTableRow_Factory::begin() {
  return m_idMap.begin();
}

std::map<Step::StepId,Step::BaseObject*>::iterator IfcTableRow_Factory::end() {
  return m_idMap.end();
}

IfcTableRow *IfcTableRow_Factory::get(Step::StepId id) {
  IfcTableRow *value;
  std::map<Step::StepId,Step::BaseObject*>::iterator it = m_idMap.find(id);
  if (it != m_idMap.end()) {
    value = static_cast< IfcTableRow * > (it->second);
  }
  else {
    LOG_ERROR("IfcTableRow_Factory::get() : Key not found.");
    return NULL;
  }
  if (value) {
    return value;
    ...
    return static_cast< IfcTableRow * > (create(id));
  }
}

Step::BaseObject *IfcTableRow_Factory::create(Step::StepId id) {
  IfcTableRow *ret = new IfcTableRow(m_model->getArgs(id));
  ret->set_key(id);
...
Step::BaseObject *IfcTableRow_Factory::create(STEP_MAP<Step::StepId, Step::BaseObjectPtr >::iterator it) {
  IfcTableRow *ret = new IfcTableRow(it->second->getArgs());
...
Step::BaseObject *IfcTableRow_Factory::create(std::map<Step::StepId, Step::BaseObject*>::iterator it) {
  IfcTableRow *ret = new IfcTableRow(m_model->getArgs(it->first));
  ret->set_key(it->first);
  m_model->registerObject(it->first, ret);
  it->second = ret;
  return ret;
...
IfcTableRow *IfcTableRow_Factory::generate() {
  return static_cast< IfcTableRow * > (create(m_model->getNewId()));
}

IfcTableRow *IfcTableRow_Factory::find(Step::StepId id) {
  std::map<Step::StepId,Step::BaseObject*>::iterator it = m_idMap.find(id);
  if (it != m_idMap.end()) {
    return static_cast< IfcTableRow * > (it->second);
  }
  else {
    return NULL;
  }
