...
IfcMonetaryUnit::IfcMonetaryUnit(Step::SPFData *args) : Step::BaseObject(args) {
...
const char *IfcMonetaryUnit::type() {
...
IFC2X3_DLL_DEF Step::ClassType IfcMonetaryUnit::s_type = new Step::ClassType_class("IfcMonetaryUnit");
IfcMonetaryUnit_Factory::IfcMonetaryUnit_Factory() {
}
...
void IfcMonetaryUnit_Factory::clear(bool b) {
}

std::map<Step::StepId,Step::BaseObject*>::iterator IfcMonetaryUnit_Factory::begin() {
  return m_idMap.begin();
}

std::map<Step::StepId,Step::BaseObject*>::iterator IfcMonetaryUnit_Factory::end() {
  return m_idMap.end();
}

IfcMonetaryUnit *IfcMonetaryUnit_Factory::get(Step::StepId id) {
  IfcMonetaryUnit *value;
  std::map<Step::StepId,Step::BaseObject*>::iterator it = m_idMap.find(id);
  if (it != m_idMap.end()) {
    value = static_cast< IfcMonetaryUnit * > (it->second);
  }
  else {
    LOG_ERROR("IfcMonetaryUnit_Factory::get() : Key not found.");
    return NULL;
  }
  if (value) {
    return value;
    ...
    return static_cast< IfcMonetaryUnit * > (create(id));
  }
}

Step::BaseObject *IfcMonetaryUnit_Factory::create(Step::StepId id) {
  IfcMonetaryUnit *ret = new IfcMonetaryUnit(m_model->getArgs(id));
  ret->set_key(id);
...
Step::BaseObject *IfcMonetaryUnit_Factory::create(STEP_MAP<Step::StepId, Step::BaseObjectPtr >::iterator it) {
  IfcMonetaryUnit *ret = new IfcMonetaryUnit(it->second->getArgs());
...
Step::BaseObject *IfcMonetaryUnit_Factory::create(std::map<Step::StepId, Step::BaseObject*>::iterator it) {
  IfcMonetaryUnit *ret = new IfcMonetaryUnit(m_model->getArgs(it->first));
  ret->set_key(it->first);
  m_model->registerObject(it->first, ret);
  it->second = ret;
  return ret;
...
IfcMonetaryUnit *IfcMonetaryUnit_Factory::generate() {
  return static_cast< IfcMonetaryUnit * > (create(m_model->getNewId()));
}

IfcMonetaryUnit *IfcMonetaryUnit_Factory::find(Step::StepId id) {
  std::map<Step::StepId,Step::BaseObject*>::iterator it = m_idMap.find(id);
  if (it != m_idMap.end()) {
    return static_cast< IfcMonetaryUnit * > (it->second);
  }
  else {
    return NULL;
  }
