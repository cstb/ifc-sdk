...
bool IfcAnnotationOccurrence::init(Step::InstanciateIf *instanciateIf) {
    bool status = IfcStyledItem::init(instanciateIf);
    std::string arg;
    if (!status) {
        return false;
    }
...
IFC2X3_EXPORT Step::ClassType IfcAnnotationOccurrence::s_type("IfcAnnotationOccurrence","IfcStyledItem");
